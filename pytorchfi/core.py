    ...
    def declare_weight_fi(self, **kwargs):
        ...
        if kwargs:
            if "function" in kwargs:
                ...
                self.CORRUPT_BATCH = kwargs.get("k", -1)
        ...
        if type(corrupt_layer) == list:
            ...
            for ind in range(len(corrupt_layer)):                    

                corrupt_idx = [corrupt_k[ind], corrupt_c[ind], corrupt_kH[ind], corrupt_kW[ind]]
                curr_layer = 0

                for module in self.CORRUPTED_MODEL.modules():
                    if isinstance(module, nn.Conv2d):
                        for name, param in module.named_parameters():
                            if "weight" in name:# and "features" in name:
                                if curr_layer == corrupt_layer[ind]:
                                    corrupt_idx = (
                                        tuple(corrupt_idx)
                                    ...
                                    if CUSTOM_INJECTION:
                                        ...
                                        corrupt_value = CUSTOM_FUNCTION(param.data[corrupt_idx], ind)
                    ...
                    for name, param in module.named_parameters():
                        if "weight" in name:# and "features" in name:
                            if curr_layer == corrupt_layer:
                                ...
                                if CUSTOM_INJECTION:
                                    # corrupt_value = CUSTOM_FUNCTION(param.data, corrupt_idx)
                                    corrupt_value = CUSTOM_FUNCTION(param.data[corrupt_idx], -1)
        ...
        #         i=0
        #         if type(corrupt_layer) == list:
        #             for i in range(len(corrupt_layer)):                    
        #                 if curr_layer == corrupt_layer[i]:
        #                     for name, param1 in param.named_parameters():
        #                         if CUSTOM_INJECTION:
        #                             CUSTOM_FUNCTION(param1.data, i)
        #         else:
        #             if curr_layer == corrupt_layer:
        #                 for name, param2 in param.named_parameters():
        #                     # corrupt_idx = (
        #                     #     tuple(corrupt_idx)
        #                     #     if isinstance(corrupt_idx, list)
        #                     #     else corrupt_idx
